...
| Plain CSS | Statically referenced scope-local stylesheets; simple class selectors and supported declarations share the signal model; conditional and compound signal-bearing selectors become explicit coverage loss |
...
| Repository graph | Oxc-contextual JSX render edges, relative imports, inherited tsconfig path aliases, workspace package exports, wildcard/named barrels and cycles, literal dynamic/lazy imports, routes, archetypes, approved primitives, and file-level Finding impact sites |
...
- runtime CVA selections plus complete symbolic variant constraints;
